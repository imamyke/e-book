        ...
        >
          ...
          <div class="card-section-title-wrapper">
            ...
            <div
              ...
              v-if="currentSectionIndex">{{currentSectionIndex}} / {{currentSectionTotal}}
            </div>
          </div>
          <div class="setting-progress">
            <div class="progress-wrapper">
              <input
                class="progress" type="range"
                max="100"
                min="0"
                step="1"
...
export default {
  ...
  setup (props, { emit }) {
    ...
    const speakCardVisible = ref(false)
    const readHeight = ref(0)
    const progress = ref(0)

    const refreshProgress = (p) => {
      progress.value = p
      ctx.$refs.progress.style.backgroundSize = `${progress.value}% 100%`
    }
    const updateText = (promise) => {
      promise.then(() => { // 改
        const currentPage = props.book?.rendition.currentLocation()
        const cfibase = props.section.cfiBase
        const cfistart = currentPage.start.cfi.replace(/.*!/, '').replace(/\)/, '')
        const cfiend = currentPage.end.cfi.replace(/.*!/, '').replace(/\)/, '')
        props.book.getRange(`epubcfi(${cfibase}!,${cfistart},${cfiend})`).then(range => {
    ...
    const togglePlay = () => {
      ctx.$parent.togglePlay()
    }
    const prev = () => {
      if (props.currentSectionIndex > 1) {
        updateText(props.book?.rendition.prev())
        emit('update:currentSectionIndex', props.currentSectionIndex - 1)
        ctx.$parent.resetPlay()
        setTimeout(() => {
          ctx.$parent.togglePlay()
        }, 500)
      }
    ...
    const next = () => {
      if (props.currentSectionIndex < props.currentSectionTotal) {
        updateText(props.book.rendition.next())
        emit('update:currentSectionIndex', props.currentSectionIndex + 1)
        ctx.$parent.resetPlay()
        setTimeout(() => {
          ctx.$parent.togglePlay()
    ...
    const display = () => {
      if (props.book.rendition) {
        const rendition = props.book.renderTo('book-read', {
          width: window.innerWidth > 640 ? 640 : window.innerWidth,
          height: readHeight.value,
          method: 'default'
        })
        rendition.display(props.section.href)
      } else {
        props.book.rendition.display(props.section.href)
      }
    }
    const onProgressChange = (p) => {
      console.log(progress)
    }
    const onProgressInput = (p) => {
      progress.value = p
      ctx.$refs.progress.style.backgroundSize = `${progress.value}% 100%`
    }
    const hide = () => {
      speakCardVisible.value = false
      visible.value = false
    }
    const show = () => {
      visible.value = true
      speakCardVisible.value = true
      refresh()
      nextTick(() => {
        display()
      })
    ...
    const refresh = () => {
      readHeight.value = window.innerHeight * 0.9 - realPx(40) - realPx(54) - realPx(46) - realPx(48) - realPx(60) - realPx(44)
      ctx.$refs.bookWrapper.style.height = readHeight.value + 'px'
    }
    return {
      visible,
      speakCardVisible,
